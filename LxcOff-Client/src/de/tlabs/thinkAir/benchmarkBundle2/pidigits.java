...
/**
 * Each program should use the same step-by-step spigot algorithm to calculate
 * digits of Pi. Each program should calculate the first N digits of Pi print
 * the digits 10-to-a-line, with the running total of digits calculated<br/>
 * 
 * TODO: requires GMP library, cannot make it to work now
 * 
 * @author Andrius
 * 
 */
public class pidigits extends Remoteable implements Benchmark {
	/**
	 * 
	 */
	private static final long serialVersionUID = 2362412650113953068L;

	final static int FOUR = 0, BQ = 1, BR = 2,
			BT = 3, // plain int values
			// GMP integers
	...
	private pidigits(int n, ExecutionController controller) {
		this.n = n;
	...
	public class exec implements Runnable {
		...
		public void run() {
			_run();
			acquire(moreWork, 1); // leave the thread spinning until more work
			// arrives - unparking takes ages on some
			// boxes
		}

		public void _run() {
			if (seq_tasks != null) {
				for (exec r : seq_tasks)
					r._run();
				allDone.release();
				return;
			}
			...
			// don't care about the CPU going wild; it would be idle otherwise
			// anyway
			acquire(sema[op1], 1);
			sema[op1].release();
			acquire(sema[op2], 1);
			...
			if (instr == MUL) {
				GmpUtil
						.mpz_mul_si(values[dest], values[op1],
								(int) values[op2]);
			} else if (instr == ADD) {
				GmpUtil.mpz_add(values[dest], values[op1], values[op2]);
			} else if (instr == DIV_Q_R) {
				GmpUtil.mpz_tdiv_qr(values[dest], values[op3], values[op1],
						values[op2]);
				sema[op3].release();
			}
	...
	// bs == 0, hence s == 0 and multiplications involving bs and s aren't here
	// (br*s, bt*s)
	...
	// the latter is the same as computing quotient and remainder of ( q*4 + r
	// )/( s*4 + t ); if the remainder is greater or equal to q,
	...
	final exec[] COMPOSE_L = new exec[] {
			...
			new exec(new exec[] { new exec(ADD, ES1, U, R),
					new exec(DIV_Q_R, ER, ER1, ES1, T) }), // DIV_Q_R is approx
			// the same cost as
			// two muls
	...
	private boolean multi_threaded_compute(exec[] code, int bq, int br, int bt,
			boolean compare) {
		...
		code[0]._run();
		boolean r = GmpUtil.mpz_cmp(values[ER1], values[Q]) >= 0; // ER1 >= Q
		// means the
		// remainder
		// of
		// (4*q+r)/t
	...
	@Remote
	void localpidigits(boolean isWarm) {
		int k = 1;
		for (int i = V; i < values.length; ++i)
			values[i] = GmpUtil.mpz_init();
		...
		for (int i = 0; i < sema.length; ++i)
			sema[i] = new Semaphore(0); // these are initially unavailable
		...
		for (;;) {
			if (multi_threaded_compute(COMPOSE_L, k, 4 * k + 2, 2 * k + 1, true)) {
				...
				multi_threaded_compute(COMPOSE_R, 10, -10 * y, 1, false); // compare
				// ==
				// false
				// -
				// computation
				...
				acquire(allDone, COMPOSE_R.length); // wait for the COMPOSE_R to
				// complete

				if (r) {
					for (int i = V; i < values.length; ++i)
						GmpUtil.mpz_clear(values[i]); // don't have to be this
					// nice in a one-shot
					// run
					return;
				}
	...
	public void main(int n) {
		pidigits m = new pidigits(n, controller);
	...
	public void copyState(Remoteable arg0) {
		pidigits remoteState = (pidigits) arg0;
		// Do not really need to restore the other values since they are only
		// used after the decision point
		this.strBuf = remoteState.strBuf;
	}
	
	void pidigits (boolean isWarm) {
	       Method toExecute;
	       Class<?>[] paramTypes = {boolean.class};
	       Object[] paramValues = { isWarm};
	       try {
	           toExecute = this.getClass().getDeclaredMethod("localpidigits", paramTypes);
...
class GmpUtil {
	...
	static native void mpz_mul_si(long dest, long src, int val);

	static native void mpz_add(long dest, long src, long src2);
	...
	static native void mpz_tdiv_qr(long q, long r, long n, long d);
}