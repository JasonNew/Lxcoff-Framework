...
/**
 * The Meteor Puzzle board is made up of 10 rows of 5 hexagonal Cells. There are
 * 10 puzzle pieces to be placed on the board, we'll number them 0 to 9. Each
 * puzzle piece is made up of 5 hexagonal Cells. As different algorithms may be
 * used to generate the puzzle solutions, we require that the solutions be
 * printed in a standard order and format. Here's one approach - working along
 * each row left to right, and down the board from top to bottom, take the
 * number of the piece placed in each cell on the board, and create a string
 * from all 50 numbers, for example the smallest puzzle solution would be
 * represented by
 * 
 * 00001222012661126155865558633348893448934747977799
 * 
 * Print the smallest and largest Meteor Puzzle 50 character solution string in
 * this format to mimic the hexagonal puzzle board:
 * 
 * <pre>
 * 0 0 0 0 1
 *  2 2 2 0 1
 ...
 * </pre>
 * 
 * The command line parameter N should limit how many solutions will be found
 * before the program halts, so that you can work with just a few solutions to
 * debug and optimize your program.
 * 
...
public final class meteor {
	...
	static final int g_firstRegion[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x01,
			0x06, 0x07, 0x08, 0x01, 0x02, 0x03, 0x0c, 0x01, 0x0e, 0x0f,

			0x10, 0x01, 0x02, 0x03, 0x04, 0x01, 0x06, 0x07, 0x18, 0x01, 0x02,
			0x03, 0x1c, 0x01, 0x1e, 0x1f };

	static final int g_flip[] = { 0x00, 0x10, 0x08, 0x18, 0x04, 0x14, 0x0c,
			0x1c, 0x02, 0x12, 0x0a, 0x1a, 0x06, 0x16, 0x0e, 0x1e,

			0x01, 0x11, 0x09, 0x19, 0x05, 0x15, 0x0d, 0x1d, 0x03, 0x13, 0x0b,
			0x1b, 0x07, 0x17, 0x0f, 0x1f, };

	static final int[] s_firstOne = { 0, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0,
			1, 0,
	...
	// -----------------------
	static class Board {
		static final int L_EDGE_MASK = ((1 << 0) | (1 << 5) | (1 << 10)
				| (1 << 15) | (1 << 20) | (1 << 25) | (1 << 30));
		static final int R_EDGE_MASK = L_EDGE_MASK << 4;
		static final int TOP_ROW = (1 << N_COL) - 1;
		static final int ROW_0_MASK = TOP_ROW | (TOP_ROW << 10)
				| (TOP_ROW << 20) | (TOP_ROW << 30);
	...
	static class Piece {
		...
		static final int BaseVecs[] = { 0x10f, 0x0cb, 0x1087, 0x427, 0x465,
